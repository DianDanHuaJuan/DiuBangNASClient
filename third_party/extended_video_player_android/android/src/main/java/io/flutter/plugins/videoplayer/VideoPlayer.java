...
final class VideoPlayer {
  ...
  private DataSource.Factory buildDataSourceFactory(
      @NonNull Context context, @NonNull Uri uri, @NonNull Map<String, String> httpHeaders) {
    final OkHttpClient trustedClient = TrustedServerRegistry.findClient(uri);
    if (trustedClient != null) {
      return buildTrustedDataSourceFactory(context, trustedClient, httpHeaders);
    }
  ...
  @NonNull
  private DataSource.Factory buildTrustedDataSourceFactory(
      @NonNull Context context,
      @NonNull OkHttpClient trustedClient,
      @NonNull Map<String, String> httpHeaders) {
    final boolean httpHeadersNotEmpty = !httpHeaders.isEmpty();
    final String userAgent =
        httpHeadersNotEmpty && httpHeaders.containsKey(USER_AGENT)
    ...
    final OkHttpDataSource.Factory okHttpDataSourceFactory =
        new OkHttpDataSource.Factory(trustedClient).setUserAgent(userAgent);
    if (httpHeadersNotEmpty) {
      okHttpDataSourceFactory.setDefaultRequestProperties(httpHeaders);
    }
    return new DefaultDataSource.Factory(context, okHttpDataSourceFactory);
  }
